...
import gradio
from gradio.components import Image as ig
from gradio.components import Label as lbl
import keras
import numpy as np
...
train = get_data(r'archive\chest_xray\train')
test = get_data(r'archive\chest_xray\test')
val = get_data(r'archive\chest_xray\val')
...
model.save_weights(r'Model\pneumoniaDectionModel')
...
img = ig(shape=(150, 150))
label = lbl(num_top_classes=1)

interface = gradio.Interface(fn = pneumoniaPrediction, title = "Pneumonia Detection using Chest X-RAY",
                             inputs = img,
                             outputs= label,
